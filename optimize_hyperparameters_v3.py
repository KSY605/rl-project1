...
from custom_walker2d_v3_optuna import CustomEnvWrapper

# 최적화를 위한 상수
N_TRIALS = 100  # 총 시도 횟수 (더 많은 하이퍼파라미터를 탐색하므로 늘리는 것을 권장)
N_TRAIN_TIMESTEPS = 30000  # 각 trial 당 학습 스텝 수
...
def objective(trial: optuna.Trial) -> float:
    ...
    보상 가중치와 PPO 하이퍼파라미터를 제안하고, 모델을 학습시킨 후, 성능을 평가하여 반환합니다.
    """
    print(f"\n===== Trial {trial.number} 시작 =====")

    # 1. 최적화할 보상 가중치의 범위를 제안합니다.
    reward_weights = {
    ...
    print(f"제안된 보상 가중치: {reward_weights}")
    print(f"제안된 PPO 하이퍼파라미터: {ppo_hyperparams}")


    # 3. 제안된 하이퍼파라미터로 커스텀 환경을 생성합니다.
    env = make_vec_env(
        CustomEnvWrapper,
        n_envs=N_ENVS,
    ...
    # 4. PPO 모델을 생성하고 학습시킵니다.
    model = PPO(
        "MlpPolicy",
        env,
        verbose=0,
        **ppo_hyperparams # 제안된 PPO 하이퍼파라미터를 적용
    )
    ...
    # 6. 평가 결과를 반환합니다. Optuna는 이 값을 최대화하는 방향으로 탐색합니다.
...
if __name__ == "__main__":
    ...
    try:
        # 최적화를 실행합니다.
        study.optimize(objective, n_trials=N_TRIALS, timeout=3600) # 2시간 제한
